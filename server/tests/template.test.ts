...
describe("/api/assessments/template", () => {
  it("generates xlsx template", async () => {
    const res = await request(app)
      ...
      .query({ subject: "Mathematics", class: "JHS 1", academicYear: "2025/2026", term: "Term 1" });
  ...
  it("generates csv template", async () => {
    const res = await request(app)
      .get("/api/assessments/template")
      .query({ subject: "Mathematics", class: "JHS 1", academicYear: "2025/2026", term: "Term 1", format: "csv" });
    expect([200, 400, 500]).toContain(res.status);
    if (res.status === 200) {
